...
- **Rule:** the legacy folder layout (French folder names) is converted by a standalone one-time script, then retired — no permanent dual-format read path in `adapters/storage/`. The new layout's path/naming scheme is declared once (e.g. as constants in `adapters/storage/`) and imported by both the migration script and the repositories, rather than each independently choosing names that happen to need to match. Settings persistence needs no equivalent conversion: `SettingsRepository` (Story 1.5) is one JSON file per `(scope, key)` with code-level defaults, already fully superseding the legacy single-CSV `entité,nom,valeur` format — nothing on that side survives to migrate. The same one-time script also mints and writes AD-6's `MazeId` header line for every legacy **classic, saved-random, and creation** maze it converts (all three predate that concept, per AD-3's `ID_ELIGIBLE_KINDS`), reusing `MazeRepository`'s own save path (folder creation and writer alike) rather than a bespoke migration-script serializer, so migrated mazes are Personal-Records-eligible from day one, with the same `MazeId` shape and position a freshly-saved maze would get. Per-folder `#_Doc_index.csv` index files are not migrated — the new layout has no index-file concept; `MazeRepository.list_names()` derives its listing from directory contents directly. `[Max's choice — resolves PRD Open Question 4; MazeId backfill added to unblock FR-27; narrowed to drop settings-file migration by a 2026-09-10 course correction]`
