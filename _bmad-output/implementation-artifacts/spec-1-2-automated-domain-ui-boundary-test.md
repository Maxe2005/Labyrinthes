...
- `pyproject.toml` -- adds `pythonpath = ["."]` under `[tool.pytest.ini_options]` so plain `pytest` (not only `python -m pytest`) can resolve `tests/test_architecture_boundaries_scanner.py`'s `from tests.test_architecture_boundaries import ...` (see Spec Change Log)
...
- [x] [Review][Patch] Fixture-regression suite doesn't actually guard against the historical "`common/` omitted" bug it claims to — that bug lived in `test_tkinter_does_not_import_storage_adapters`'s hardcoded subpackage tuple, not in the shared `find_forbidden_imports`/`resolve_imported_modules` helper the fixtures exercise [tests/test_architecture_boundaries_scanner.py]
- [x] [Review][Patch] `test_submodule_style_import_of_forbidden_target_is_detected` actually exercises a relative import, duplicating `test_relative_import_is_detected`'s mechanism, rather than the absolute submodule-style case from the Design Notes (`from labyrinthes.adapters.tkinter import builder`) [tests/test_architecture_boundaries_scanner.py:435]
- [x] [Review][Patch] Code Map says `pyproject.toml -- no changes needed`, but the diff adds `pythonpath = ["."]`; neither the Code Map nor the Spec Change Log documents this third review loopback [pyproject.toml:32]
- [x] [Review][Defer] `TYPE_CHECKING`-guarded imports are not excluded from the AST scan and would false-positive once `domain/`/`application/` first use type-only imports — not currently reachable, no code does this yet [tests/test_architecture_boundaries.py] — deferred, pre-existing scope boundary; revisit when a story first needs a `TYPE_CHECKING` import in `domain/`/`application/`
- [x] [Review][Defer] Minor untested/low-probability edge branches: bare `from . import x` (logic re-read, correct), a relative import climbing above the top-level package (Python itself would crash on this at runtime), a directory shadowed by a same-named file [tests/test_architecture_boundaries.py] — deferred, no reachable consequence today
- [x] [Review][Defer] `_format_violations` reports no line number, only file + module — a debuggability nicety, not required by any AC [tests/test_architecture_boundaries.py:117] — deferred, optional improvement
- [x] [Review][Defer] A future `adapters/tkinter/__init__.py` barrel re-export would bypass the screen-isolation check [tests/test_architecture_boundaries.py] — deferred, speculative/forward-looking, no such file exists yet
...
- **Finding (code review, patch):** the new `tests/test_architecture_boundaries_scanner.py` module needs `from tests.test_architecture_boundaries import ...` to resolve under plain `pytest` (not only `python -m pytest`, which prepends cwd to `sys.path` on its own). **Amended:** added `pythonpath = ["."]` under `[tool.pytest.ini_options]` in `pyproject.toml`; updated the Code Map, which had said no `pyproject.toml` change was needed.
- **Finding (code review, patch):** Blind Hunter found that the fixture-regression suite didn't actually guard against the exact historical bug it claimed to (iteration 2's `common/` omission), because that bug lived in `test_tkinter_does_not_import_storage_adapters`'s inline subpackage tuple, not in the shared `find_forbidden_imports`/`resolve_imported_modules` helper the fixtures exercise. **Amended:** extracted `ADAPTERS_TKINTER_SUBPACKAGES` as a named constant used by the production test, and added `test_storage_check_covers_all_four_tkinter_subpackages` in the fixture file to pin its exact contents.
- **Finding (code review, patch):** Acceptance Auditor found, and verified by reverting the name-based resolution line, that `test_submodule_style_import_of_forbidden_target_is_detected` actually exercised a relative import (`from ..builder import x`), duplicating `test_relative_import_is_detected`'s mechanism rather than the absolute submodule-style case from the Design Notes (`from labyrinthes.adapters.tkinter import builder`). **Amended:** rewrote the fixture to use that absolute form.
