...
Deliver the Game/Player's core play loop end-to-end: browsing and picking a maze (classic library or freshly generated random), and playing it with configurable Levels, Difficulty, HARD mode, movement style, timer, confirmation prompts, and appearance — matching and fixing the legacy player's mature-but-buggy feature set. This is UJ-2's spine (short of Personal Records, which lands in Epic 6) and the first fully playable slice of the rewrite.
...
- Timer: the Time HUD chip updates continuously during a run, using the shared `Duration` type (Story 1.1) — the same type `Record.time` (Epic 6) will use. An optional configurable time limit, when reached before the exit is found, shows a non-modal inline failure message ("Time's up — the exit wasn't reached.") with restart/continue still reachable; with no limit configured, elapsed time appears in the win banner. (The legacy `Chrono` class was fully implemented but never wired up — this epic finishes wiring it.)
...
- Records (`RecordsService.record_completion(...)` on win) are explicitly Epic 6's concern, not this epic's — Story 2.4's win detection stops at marking the run solved and showing the win banner.
...
- This epic's `Maze` kind/`MazeId` handling depends on Story 1.1's domain model and interacts with Epic 3's Builder (FR-8 `Test in Player` bypasses this epic's selection screen entirely; FR-19 `Edit in Builder` depends on this epic's `saved-random`/`classic` kind distinction) and Epic 6 (Personal Records reads the same `Maze.kind`/`MazeId`/`Level`/`Difficulty`/`Duration` types this epic establishes usage of, and Story 2.4's win detection is exactly where Epic 6's `RecordsService.record_completion(...)` call will be added).
