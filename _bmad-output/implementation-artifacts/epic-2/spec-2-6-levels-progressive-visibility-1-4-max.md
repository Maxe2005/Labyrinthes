...
**Never:** No Difficulty control (that is Story 2.7 — including its Level-1-disable gating and its final unified threshold formula). No HARD-mode fog/status light (2.8), no timer/timeout (2.9), no confirmation prompts on Level change (2.10), no first-activation explainers (Epic 6 — but leave the Level control's location/state in a shape that won't block later ⓘ-anchor wiring). No change to `composition_root.py`, `screen.py`, `settings_keys.py`, or `keybindings.py`. No new persisted setting. No change to `PlayerSession`'s existing `mode`/`speed`/leg fields or to `attempt_move`/`Duration`. No redraw on every animation sub-step — structure redraws only when the visibility object actually changes (partition entered, threshold reset, wall discovered, contour toggle, Level change).
