...
- `MazeKind.SKETCH` has `id=None` eligibility; only `CLASSIC`/`SAVED_RANDOM` eligible kinds receive a `MazeId` on save
...
- [x] [Review][Patch] Global letter shortcuts (`b`/`c`/`p`) leak into `NewMazeDialog`'s `Entry` fields via `bind_all()`, unlike `GenerateRandomDialog`'s own `<KeyPress-n>`/`<KeyPress-N>` "break" guard [`src/labyrinthes/adapters/tkinter/common/new_maze_dialog.py:115`]
- [x] [Review][Patch] `status: 'done'` in this spec's frontmatter contradicts `sprint-status.yaml`'s `review` for the same story in the same diff [`_bmad-output/implementation-artifacts/epic-3/spec-3-1-new-maze-open-a-sketch.md:5`]
- [x] [Review][Patch] Spec Change Log Iteration 1 claims `test_new_maze_dialog.py` has "16 tests" but the file defines 13 `test_` functions [`tests/adapters/tkinter/common/test_new_maze_dialog.py`]
- [x] [Review][Patch] Typo "elidible" should read "eligible" in Boundaries & Constraints [`_bmad-output/implementation-artifacts/epic-3/spec-3-1-new-maze-open-a-sketch.md:26`]
- [x] [Review][Defer] Spec's Iteration 1 justification for omitting `transient()`/`grab_set()` misreads `confirm_dialog.py` (which does call `self.transient(parent)`) as its evidence; `NewMazeDialog`'s actual behavior correctly mirrors its real model `GenerateRandomDialog` (which also skips `transient()`), so no code change is needed, only the spec's citation is wrong [`src/labyrinthes/adapters/tkinter/common/confirm_dialog.py:113`] — deferred, pre-existing
- [x] [Review][Defer] `final_revision` left blank in this spec's frontmatter despite two completed review iterations documented in the Spec Change Log [`_bmad-output/implementation-artifacts/epic-3/spec-3-1-new-maze-open-a-sketch.md:10`] — deferred, pre-existing
