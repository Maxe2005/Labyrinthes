...
- Only persistence is a port; rendering is not. `MazeRepository` and `SettingsRepository` (plus `RecordsRepository`, added in Epic 5) are the sole ports in `application/`, each with exactly one implementation under `adapters/storage/`, used by every screen through the shell's single composition root.
...
- This entire epic is a prerequisite for Epic 2, Epic 3, and Epic 5, all of which consume `MazeRepository`/`SettingsRepository`, the router, and the `common/` toolkit established here. Epic 4's migration script also reuses this epic's path/naming constants and `MazeRepository`'s `MazeId` writer.
