...
- [x] `src/labyrinthes/adapters/storage/maze_id_minting.py` -- add `mint_maze_id() -> MazeId` -- single shared minting function Story 1.4's `save()` and Epic 5's migration script both call
...
- addressed_findings:
  ...
  - `[low]` `[patch]` `adapters/storage/__init__.py`'s `__all__` omitted `MAZE_FILE_SUFFIX`, even though `paths.py`'s own docstring frames the whole module (not just `maze_file_path`) as what Epic 5's migration script will import — added it to the import and `__all__`
