...
def test_initial_render_shows_the_maze_canvas_with_entry_exit_and_ball(
    ...
    maze = _classic_maze()
    screen = GameplayScreen(tk_root, maze, Theme.LIGHT, maze_repository=fake_maze_repository)
...
def test_move_through_an_open_passage_updates_the_ball_and_pos_chip(tk_root, fake_maze_repository):
...
def test_movement_shortcuts_are_registered(tk_root, fake_maze_repository):
    GameplayScreen(tk_root, _classic_maze(), Theme.LIGHT, maze_repository=fake_maze_repository)
...
def test_move_is_a_no_op_while_a_text_entry_holds_focus(tk_root, fake_maze_repository):
...
def test_win_banner_text_reports_the_elapsed_clock_string(tk_root, fake_maze_repository):
    maze = _open_maze(width=2)
    screen = GameplayScreen(tk_root, maze, Theme.LIGHT, maze_repository=fake_maze_repository)
...
def test_continue_button_is_not_a_primary_pill(tk_root, fake_maze_repository):
...
def test_reaching_the_exit_cancels_the_pending_tick_job(tk_root, fake_maze_repository):
    maze = _open_maze(width=2)
    screen = GameplayScreen(tk_root, maze, Theme.LIGHT, maze_repository=fake_maze_repository)
...
def test_move_after_solved_is_a_no_op(tk_root, fake_maze_repository):
    maze = _open_maze(width=2)
    screen = GameplayScreen(tk_root, maze, Theme.LIGHT, maze_repository=fake_maze_repository)
    screen._on_move(Direction.RIGHT)
    assert screen._session.solved is True
...
def test_continue_click_destroys_the_banner_but_keeps_solved_true(tk_root, fake_maze_repository):
    maze = _open_maze(width=2)
    screen = GameplayScreen(tk_root, maze, Theme.LIGHT, maze_repository=fake_maze_repository)
    screen._on_move(Direction.RIGHT)
...
def test_on_tick_updates_the_time_chip_and_reschedules(tk_root, fake_maze_repository):
    screen = GameplayScreen(
...
def test_destroying_the_screen_cancels_the_pending_tick_job(tk_root, fake_maze_repository):
    screen = GameplayScreen(
        tk_root, _classic_maze(), Theme.LIGHT, maze_repository=fake_maze_repository
    )
    assert screen._tick_job is not None
...
def test_confirming_save_updates_the_screens_own_maze_and_hides_save(tk_root, fake_maze_repository):
    screen = GameplayScreen(
        tk_root, _generated_maze(), Theme.LIGHT, maze_repository=fake_maze_repository
    )
...
def test_confirming_save_also_updates_the_sessions_own_maze(tk_root, fake_maze_repository):
...
def test_confirming_save_notifies_on_kind_changed_with_the_new_kind(tk_root, fake_maze_repository):
...
def test_on_kind_changed_defaults_to_none_and_a_save_still_succeeds(tk_root, fake_maze_repository):
    screen = GameplayScreen(
        tk_root, _generated_maze(), Theme.LIGHT, maze_repository=fake_maze_repository
    )
...
def test_confirming_save_does_not_rebuild_the_hud_or_canvas(tk_root, fake_maze_repository):
...
def test_save_shortcut_is_registered_while_the_save_button_exists(tk_root, fake_maze_repository):
    GameplayScreen(tk_root, _generated_maze(), Theme.LIGHT, maze_repository=fake_maze_repository)
