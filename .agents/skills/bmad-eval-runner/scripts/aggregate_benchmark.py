...
def summarize_config(records: list[dict]) -> dict:
    ...
    return {
        "runs": len(records),
        "metrics": {name: summarize_metric(vals) for name, vals in sorted(by_metric.items())},
    }
...
def main(argv: list[str] | None = None) -> int:
    ...
    p.add_argument(
        "--runs", type=Path, help="summarize one config (JSON file or dir of timing.json)"
    )
    p.add_argument("--baseline", type=Path, help="baseline config for a two-config comparison")
    p.add_argument("--variant", type=Path, help="variant config for a two-config comparison")
    p.add_argument(
        "--self-test", action="store_true", help="run the built-in fixture self-test and exit"
    )
