...
def build_argv(invocation: list, prompt: str, cwd: str) -> list[str]:
    ...
    for tok in invocation:
        tok = str(tok)
        tok = tok.replace("{prompt}", prompt).replace("{query}", prompt).replace("{cwd}", cwd)
        argv.append(tok)
    return argv


def build_case_env(
    adapter: Mapping | None, home_dir: Path, host_env: Mapping[str, str]
) -> dict[str, str]:
...
def read_transcript(transcript_cfg: dict, captured_stdout: bytes, cwd: Path) -> tuple[str, str]:
...
def run_case(
    ...
    transcript_text, source = read_transcript(adapter.get("transcript", {}), captured, cwd)
...
def main(argv: list[str] | None = None) -> int:
    ...
    p.add_argument(
        ...
        type=Path,
        default=None,
        help="base for resolving fixture paths; defaults to the cases file's directory",
    )
    p.add_argument(
        "--adapter",
        type=Path,
    ...
    p.add_argument(
        "--runs", type=int, default=1, help="repeats per case per config for the variance benchmark"
    )
    ...
    project_root = args.project_root.resolve() if args.project_root else cases_file.parent
    ...
    if adapter is None and not args.quiet:
        print(
            "[run_evals] no runtime adapter configured; staging cases only "
            "(no crash). Configure an adapter to execute.",
            file=sys.stderr,
        )
    ...
    if not args.quiet:
        print(
            f"[run_evals] {len(cases)} cases x {len(configs)} configs x "
    ...
    with ThreadPoolExecutor(max_workers=max(1, args.workers)) as pool:
        fut_to_case = {
            pool.submit(
                ...
                config_name,
                config_skill,
                resolve_fixtures(c.get("files", []), project_root, cases_file.parent),
            ): c
        ...
        for fut in as_completed(fut_to_case):
            ...
            if not args.quiet:
                print(
                    f"  [{res.get('status')}] {res.get('config', '?')}/"
    ...
    summary = {
        ...
        "failures": sum(
            1
            for r in results
            if r.get("status") in ("error", "timeout", "exception", "adapter-missing")
        ),
        "run_dir": str(run_dir),
        "results": results,
    }
