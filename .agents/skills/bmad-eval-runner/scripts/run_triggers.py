...
def run_query_once(
    query: str, skill_name: str, description: str, adapter: dict, stage_dir: Path, timeout: int
) -> bool:
...
def main(argv: list[str] | None = None) -> int:
    ...
    if adapter is None:
        if not args.quiet:
            print(
                "[run_triggers] no runtime adapter configured; staging only (no crash).",
                file=sys.stderr,
            )
        output = {
            "run_id": run_id,
    ...
    def run_one(idx: int, q: dict, run_idx: int) -> tuple[int, bool]:
        ...
        try:
            ...
            shutil.rmtree(
                stage / adapter.get("skill_dir", ".claude/skills").split("/")[0], ignore_errors=True
            )
    ...
    if not args.quiet:
        print(
            f"[run_triggers] {len(queries)} queries x {args.runs_per_query} runs", file=sys.stderr
        )
