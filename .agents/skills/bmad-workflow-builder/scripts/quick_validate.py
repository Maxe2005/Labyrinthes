...
def main(argv: list[str] | None = None) -> int:
    ...
    p.add_argument(
        "--allow-keys", help="comma-separated set that REPLACES the default allowed keys"
    )
    p.add_argument("--max-name", type=int, default=64, help="max name length (default 64)")
    p.add_argument(
        "--max-desc", type=int, default=1024, help="max description length (default 1024)"
    )
    ...
    if not skill_md.is_file():
        print(
            json.dumps(
                {"ok": False, "errors": [{"check": "path", "message": f"{skill_md} not found"}]}
            )
        )
        return 1
